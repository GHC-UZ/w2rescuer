	...
	normalY=0.0;
		...
		if(h[left] >= tol9 || h[right] >= tol9){			// Wet wall
			...
			if(h[left]<tol9 && hx < 0.0){	// dry-wet wall
				DU1_1[right] += (lambda1*alpha1-beta1)*e1[0];
				DU2_1[right] += 0.0;
				DU3_1[right] += 0.0;

			} else if(h[right]<tol9 && hxx < 0.0){ // wet-dry wall
				DU1_0[left] += (lambda1*alpha1-beta1)*e1[0];
				DU2_0[left] += 0.0;
				DU3_0[left] += 0.0;

			} else if(lb1l < 0.0 && lb1r > 0.0){
				lambda1minus = lb1l*(lb1r-lambda1)/(lb1r-lb1l);
				DU1_0[left] += (lambda1minus*alpha1-beta1)*e1[0];
				DU2_0[left] += (lambda1minus*alpha1-beta1)*e1[1];
				DU3_0[left] += (lambda1minus*alpha1-beta1)*e1[2];
				lambda1plus = lb1r*(lambda1-lb1l)/(lb1r-lb1l);
				DU1_1[right] += (lambda1plus*alpha1)*e1[0];
				DU2_1[right] += (lambda1plus*alpha1)*e1[1];
				DU3_1[right] += (lambda1plus*alpha1)*e1[2];

			} else if(lambda1 < 0.0){
				DU1_0[left] += (lambda1*alpha1-beta1)*e1[0];
				DU2_0[left] += (lambda1*alpha1-beta1)*e1[1];
				DU3_0[left] += (lambda1*alpha1-beta1)*e1[2];

			} else if(lambda1 >= 0.0){
				DU1_1[right] += (lambda1*alpha1-beta1)*e1[0];	
				DU2_1[right] += (lambda1*alpha1-beta1)*e1[1];
				DU3_1[right] += (lambda1*alpha1-beta1)*e1[2];
			}
			
			// Second wave
			if(h[left]<tol9 && hx < 0.0){ 		// dry-wet wall
				DU1_1[right] += (lambda2*alpha2-beta2)*e2[0];
				DU2_1[right] += 0.0;
				DU3_1[right] += 0.0;

			} else if(h[right]<tol9 && hxx < 0.0){	// wet-dry wall
				DU1_0[left] += (lambda2*alpha2-beta2)*e2[0];
				DU2_0[left] += 0.0;
				DU3_0[left] += 0.0;

			} else if(lambda2 < 0.0){
				DU1_0[left] += (lambda2*alpha2-beta2)*e2[0];
				DU2_0[left] += (lambda2*alpha2-beta2)*e2[1];
				DU3_0[left] += (lambda2*alpha2-beta2)*e2[2];

			} else if(lambda2 >= 0.0){
				DU1_1[right] += (lambda2*alpha2-beta2)*e2[0];
				DU2_1[right] += (lambda2*alpha2-beta2)*e2[1];
				DU3_1[right] += (lambda2*alpha2-beta2)*e2[2];
			}


			// Third wave
			if(h[left]<tol9 && hx < 0.0){ 		// dry-wet wall
				DU1_1[right] += (lambda3*alpha3-beta3)*e3[0];
				DU2_1[right] += 0.0;
				DU3_1[right] += 0.0;

			} else if(h[right]<tol9 && hxx < 0.0){	// wet-dry wall
				DU1_0[left] += (lambda3*alpha3-beta3)*e3[0];
				DU2_0[left] += 0.0;
				DU3_0[left] += 0.0;

			} else if(lb3l < 0.0 && lb3r > 0.0){
				lambda3minus=lb3l*(lb3r-lambda3)/(lb3r-lb3l);
				DU1_0[left] += (lambda3minus*alpha3)*e3[0];
				DU2_0[left] += (lambda3minus*alpha3)*e3[1];
				DU3_0[left] += (lambda3minus*alpha3)*e3[2];
				lambda3plus=lb3r*(lambda3-lb3l)/(lb3r-lb3l);
				DU1_1[right] += (lambda3plus*alpha3-beta3)*e3[0];
				DU2_1[right] += (lambda3plus*alpha3-beta3)*e3[1];
				DU3_1[right] += (lambda3plus*alpha3-beta3)*e3[2];

			} else if(lambda3 < 0.0){
				DU1_0[left] += (lambda3*alpha3-beta3)*e3[0];
				DU2_0[left] += (lambda3*alpha3-beta3)*e3[1];
				DU3_0[left] += (lambda3*alpha3-beta3)*e3[2];

			} else if(lambda3 >= 0.0){
				DU1_1[right] += (lambda3*alpha3-beta3)*e3[0];
				DU2_1[right] += (lambda3*alpha3-beta3)*e3[1];
				DU3_1[right] += (lambda3*alpha3-beta3)*e3[2];
			}
		...
		int top = IDX(i,j+1,nX);	
			if(h[bottom] >= tol9 || h[top] >= tol9){			// Wet wall
				...
				if(h[bottom]<tol9 && hx < 0.0){	// dry-wet wall
					DU1_1[top] += (lambda1*alpha1-beta1)*e1[0];
					DU2_1[top] += 0.0;
					DU3_1[top] += 0.0;

				} else if(h[top]<tol9 && hxx < 0.0){ // wet-dry wall
					DU1_0[bottom] += (lambda1*alpha1-beta1)*e1[0];
					DU2_0[bottom] += 0.0;
					DU3_0[bottom] += 0.0;

				} else if(lb1l < 0.0 && lb1r > 0.0){
					lambda1minus = lb1l*(lb1r-lambda1)/(lb1r-lb1l);
					DU1_0[bottom] += (lambda1minus*alpha1-beta1)*e1[0];
					DU2_0[bottom] += (lambda1minus*alpha1-beta1)*e1[1];
					DU3_0[bottom] += (lambda1minus*alpha1-beta1)*e1[2];
					lambda1plus = lb1r*(lambda1-lb1l)/(lb1r-lb1l);
					DU1_1[top] += (lambda1plus*alpha1)*e1[0];
					DU2_1[top] += (lambda1plus*alpha1)*e1[1];
					DU3_1[top] += (lambda1plus*alpha1)*e1[2];

				} else if(lambda1 < 0.0){
					DU1_0[bottom] += (lambda1*alpha1-beta1)*e1[0];
					DU2_0[bottom] += (lambda1*alpha1-beta1)*e1[1];
					DU3_0[bottom] += (lambda1*alpha1-beta1)*e1[2];

				} else if(lambda1 >= 0.0){
					DU1_1[top] += (lambda1*alpha1-beta1)*e1[0];	
					DU2_1[top] += (lambda1*alpha1-beta1)*e1[1];
					DU3_1[top] += (lambda1*alpha1-beta1)*e1[2];
				}
				
				// Second wave
				if(h[bottom]<tol9 && hx < 0.0){ 		// dry-wet wall
					DU1_1[top] += (lambda2*alpha2-beta2)*e2[0];
					DU2_1[top] += 0.0;
					DU3_1[top] += 0.0;

				} else if(h[top]<tol9 && hxx < 0.0){	// wet-dry wall
					DU1_0[bottom] += (lambda2*alpha2-beta2)*e2[0];
					DU2_0[bottom] += 0.0;
					DU3_0[bottom] += 0.0;

				} else if(lambda2 < 0.0){
					DU1_0[bottom] += (lambda2*alpha2-beta2)*e2[0];
					DU2_0[bottom] += (lambda2*alpha2-beta2)*e2[1];
					DU3_0[bottom] += (lambda2*alpha2-beta2)*e2[2];

				} else if(lambda2 >= 0.0){
					DU1_1[top] += (lambda2*alpha2-beta2)*e2[0];
					DU2_1[top] += (lambda2*alpha2-beta2)*e2[1];
					DU3_1[top] += (lambda2*alpha2-beta2)*e2[2];
				}


				// Third wave
				if(h[bottom]<tol9 && hx < 0.0){ 		// dry-wet wall
					DU1_1[top] += (lambda3*alpha3-beta3)*e3[0];
					DU2_1[top] += 0.0;
					DU3_1[top] += 0.0;

				} else if(h[top]<tol9 && hxx < 0.0){	// wet-dry wall
					DU1_0[bottom] += (lambda3*alpha3-beta3)*e3[0];
					DU2_0[bottom] += 0.0;
					DU3_0[bottom] += 0.0;

				} else if(lb3l < 0.0 && lb3r > 0.0){
					lambda3minus=lb3l*(lb3r-lambda3)/(lb3r-lb3l);
					DU1_0[bottom] += (lambda3minus*alpha3)*e3[0];
					DU2_0[bottom] += (lambda3minus*alpha3)*e3[1];
					DU3_0[bottom] += (lambda3minus*alpha3)*e3[2];
					lambda3plus=lb3r*(lambda3-lb3l)/(lb3r-lb3l);
					DU1_1[top] += (lambda3plus*alpha3-beta3)*e3[0];
					DU2_1[top] += (lambda3plus*alpha3-beta3)*e3[1];
					DU3_1[top] += (lambda3plus*alpha3-beta3)*e3[2];

				} else if(lambda3 < 0.0){
					DU1_0[bottom] += (lambda3*alpha3-beta3)*e3[0];
					DU2_0[bottom] += (lambda3*alpha3-beta3)*e3[1];
					DU3_0[bottom] += (lambda3*alpha3-beta3)*e3[2];

				} else if(lambda3 >= 0.0){
					DU1_1[top] += (lambda3*alpha3-beta3)*e3[0];
					DU2_1[top] += (lambda3*alpha3-beta3)*e3[1];
					DU3_1[top] += (lambda3*alpha3-beta3)*e3[2];
				}
...
void h_check_depth_positivity(int nCells, 
	double *h, double *DU1_0, double *DU1_1, double dx, double *dt){
    for(int ic = 0; ic < nCells; ic++){
		double aux1;

		aux1 = h[ic] - (*dt)*(DU1_0[ic]+DU1_1[ic])/dx;		
