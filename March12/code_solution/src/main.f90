...
call initial(qp,n,dx,a,g,maxl,mass,etot,be,dxi,amplitude,swl)
call primitives(qp,pv)

t=0.0d0
ite=0
call popr(qp,be,dxi,g,t,ite) 
...
!---------Loop in time----------------------
if(itime.eq.1) then
        call euler(qp,pv,be,gp,ite,massin) 
elseif(itime.eq.2) then
        call rk2(qp,pv,be,gp,ite,massin) 
else
        print*,'Only Eurer and RK2 are implemented'
        stop
endif

!-----post processing----------------------- 
t=Tt
if(t.ne.0.0d0) call popr(qp,be,dxi,g,t,ite) 
...
!*******************************************************************
subroutine primitives(qp,pv)
implicit none
integer n,i
double precision dt,dx,a,b,L,eps_h,eps_u,Mass_dry,Vol_wet
double precision qp(2,-3:n+4),pv(2,-3:n+4),threshold
common /tr/ threshold
common /bd/ a,b
common /dn/ dt,dx,n
...
do i=1,n
	if (qp(1,i).lt.eps_h) then
		...
		Vol_wet = Vol_wet + 1 ! total volume of wet areas
	endif
enddo
...
!************** Kourganov approach *******************************
      		 pv(2,i) = qp(1,i)*qp(2,i)/sqrt(max(qp(1,i)**2,eps_u)**2)
