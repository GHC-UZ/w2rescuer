...
int h_initialize_variables(int nCells, int nWalls,
	double *A, double *Q, double *h, double *q, double *u, 
	double *c, double *Fr, 
	double *B, double *P, 
	double *n, double *zb, 	
	double *DU1, double *DU2,
	double *lSW, int *solidWall){
...
int h_compute_wall_fluxes(int nWalls,
	...
	for(ic=0; ic<nWalls; ic++){	
		...
		if(h[ic] >= tol9 || h[ic+1] >= tol9){			// Wet wall
			...
			// Eigenvectors
			e1[0] = 1.;      e1[1] = uROE-cROE;
			e2[0] = 1.;      e2[1] = uROE+cROE;

			// Wave-strenght coefficients
			deltaA0 = A[ic+1]-A[ic];
			deltaQ0 = Q[ic+1]-Q[ic];
			alpha1 = (1./(2.*cROE))*(lambda2*deltaA0-deltaQ0);
			alpha2 = (-1./(2.*cROE))*(lambda1*deltaA0-deltaQ0);


			// Source term coefficients
			// Fuerza de presion en solera - Differential formulation
			So = -(zb[ic+1]-zb[ic])/dx[ic];
			Fp = g*Aav*So*dx[ic];
			Fw = -g*Aav*((h[ic+1]-h[ic])-1./Bav*deltaA0); 	
			b1P = (-1./(2.*cROE)) * (Fp+Fw);
			b2P = -b1P;

			// Fuerza de friccion en solera
			nav = 0.5*(n[ic]+n[ic+1]);
			Sf = 0.0;
			#if friction == 1
				#if Fmodel == 1 // Manning
					Sf = nav*nav*fabs(uROE)*uROE/pow(hav,4./3.); // Unit formulation

				#elif Fmodel == 2//Viscous
					Sf = 3.*mu*uROE / (rhow*g*hav); // Unit formulation
				#endif
			#endif
			...
			if(A[ic]<tol9 && Ax < 0.0){	// left dry-wet wall
				setSolidWall=-1;		
			}else if(A[ic+1]<tol9 && Axx < 0.0){ // right wet-dry wall
				setSolidWall=1;
			}else{ //wet-wet wall
				setSolidWall=0;
			}
...
int h_check_depth_posotivity(int nCells, 
...
int h_update_wetdry_fronts(int nWalls, 
	double *h, double *zb,
	double *Q, double *q, double *u, double *Fr,
	int *solidWall){
	...
	for(ic=0; ic<nWalls; ic++){	
		...
		//Topographic wet-dry fronts
		if( (h[ic] < tol9) && (zb[ic] > zsR) ){
			solidWall[ic]=-1;
		}
		if( (h[ic+1] < tol9) && (zb[ic+1] > zsL) ){
			solidWall[ic]=1;
		}		

		//Set wet-dry front momentum
		if(solidWall[ic]==-1){ //left wet-dry front
			Q[ic+1] = 0.0;
			u[ic+1] = 0.0;
			q[ic+1] = 0.0;			
			Fr[ic+1] = 0.0;	
		}
		if(solidWall[ic]==1){ //rigth wet-dry front
			Q[ic] = 0.0;
			u[ic] = 0.0;
			q[ic] = 0.0;			
			Fr[ic] = 0.0;	
